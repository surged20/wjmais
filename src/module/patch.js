...
function patchResourceBars() {
  ...
  libWrapper.register('wjmais', 'TokenDocument.prototype.getBarAttribute', function (wrapped, ...args) {
    ...
    if (attr === "Bulwark Points") {
      const value = foundry.utils.getProperty(this.actor.system, "attributes.hp.temp");
      const max = foundry.utils.getProperty(this.actor.system, "attributes.hp.tempmax");
  ...
  libWrapper.register('wjmais', 'Actor.prototype.modifyTokenAttribute', function (wrapped, ...args) {
    ...
    if (attribute === "Bulwark Points") {
      const currentValue = foundry.utils.getProperty(this.system, "attributes.hp.temp");
      const currentMax = foundry.utils.getProperty(this.system, "attributes.hp.tempmax");
  ...
  libWrapper.register('wjmais', 'game.dnd5e.canvas.Token5e.prototype._drawBar', function (wrapped, ...args) {
    if (game.actors.get(this.document.actorId).flags?.wjmais?.traits)
      return Object.getPrototypeOf(game.dnd5e.canvas.Token5e).prototype._drawBar.apply(this, args);
    else
      return wrapped(...args);
  }, 'MIXED' );

  // Display raw HP value for wildjammers
  libWrapper.register('wjmais', 'game.dnd5e.documents.TokenDocument5e.prototype.getBarAttribute', function (wrapped, ...args) {
    if (game.actors.get(this.actorId).flags?.wjmais?.traits)
      return Object.getPrototypeOf(game.dnd5e.documents.TokenDocument5e).prototype.getBarAttribute.apply(this, args);
    else
      return wrapped(...args);
  }, 'MIXED' );
}
...
function patchRollData() {
  libWrapper.register('wjmais', 'game.dnd5e.documents.Actor5e.prototype.getRollData', function (wrapped, ...args) {
